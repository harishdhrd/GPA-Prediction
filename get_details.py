...
class GetDetails(object):
    ...
    def map_id_to_reg_no(self):
        ...
        json_data = {"Batch":"4034","CourseId":"11","SemesterId":"3088"}
    ...
    def find_gpa(self,PersonID):
        """
	    cookie = requests.post(self.validateUserUrl, json={"PuniVMenulist": "manager.it@rajalakshmi.edu.in", "RoleID": 0}).headers
        cookie = cookie['Set-Cookie'].split("=")
        json_data = {"AcademicYearId":"3509","CourseId":11,"SemesterId":"3088","PersonId":PersonID}
        headers = {}
        headers = {"Referer": "http://rajalakshmi.in/UI/Modules/uniCurrentResult.aspx?FormHeading=Examination%20Result",
           "Cookie": "G_ENABLED_IDPS=google; ASP.NET_SessionId=ixsjiaxljkc1u4ab1zttydmd"}
        headers['Cookie'] = "G_ENABLED_IDPS=google; " + cookie[0]+"="+cookie[1].split(";")[0]
        ...
        cookie = requests.post("http://rajalakshmi.in/UI/Modules/Login/UniLogin.aspx/VlidateUser",json={"PuniVMenulist": "manager.it@rajalakshmi.edu.in", "RoleID": 0}).headers
        cookie = cookie['Set-Cookie'].split("=")
        ...
        response = json.loads(requests.post("http://rajalakshmi.in//UI/Modules/UniCurrentResult.aspx/GetResult",
                                            json=json_data, headers=headers).json()['d'])
        gpa, sub_count = 0, 0
        for res in response:
            if res['TOTAL'] != 0 and res['CREDITS'] != 0:
                gpa += res['TOTAL'] / res['CREDITS']
    ...
    def add_details(self):
        with open("student-data.csv","a",newline='\n') as file:
            ...
            subs = ["Computer Networks","Human Computer Interaction","Internet Programming","Principles of Artificial Intelligence","Theory of Computation","GPA"]
            ...
            for reg_no in sorted(self.reg_no):
                ...
                t1.append(gpa)
                writer.writerow(t1)
                print("Successfully Added"+reg_no)
